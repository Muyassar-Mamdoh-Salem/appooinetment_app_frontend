        ...
        {categoryList.map((cat, index) => {
          ...
          return (
            <Link
              href={`/search/${name}`}
              key={cat.id || index}
              className="flex flex-col text-center items-center p-5 bg-lime-200 m-2 rounded-lg hover:scale-110 transition-all ease-in-out cursor-pointer"
            >
              {iconUrl && (
                <Image
                  src={`http://localhost:1337${iconUrl}`}
                  width={70}
                  height={70}
                  alt={name}
                />
